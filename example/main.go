...
func main() {
	...
	chat.CheckAccepted("route", "route.json")
	...
	fmt.Println(text)  //print important modeled text
	fmt.Println(label) //print important modeled label
	texts := strings.Join(text, " ")
	//start to use that data to run commands!
	if slices.Contains(label, "route") {
		fmt.Println("Create route " + texts + "...")
